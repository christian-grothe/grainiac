...
use crate::ratavstui::{RatatuiEditor, RatatuiState};

mod ratavstui;
mod utils;
mod widgets;

const INSTANCE_NUM: usize = 2;
...
impl Enum for Hold {
    ...
    fn from_index(index: usize) -> Self {
        if index == 0 { Hold::Off } else { Hold::On }
    }
...
impl Default for Grainiac {
    fn default() -> Self {
        let (sampler, buf_output) = Sampler::new(48000.0, INSTANCE_NUM);
...
impl Default for GrainiacParams {
    fn default() -> Self {
        Self {
            editor_state: Arc::new(RatatuiState::default()),
...
impl Plugin for Grainiac {
    const NAME: &'static str = "Grainiac TUI";
    ...
    fn editor(&mut self, _async_executor: AsyncExecutor<Self>) -> Option<Box<dyn Editor>> {
        Some(Box::new(RatatuiEditor {
            state: self.params.editor_state.clone(),
            params: self.params.clone(),
            draw_data: self.buf_output.clone(),
...
impl Vst3Plugin for Grainiac {
    const VST3_CLASS_ID: [u8; 16] = *b"NIHRatatuiPlugin";
